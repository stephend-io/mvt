...
const ChannelBox = () => {
  ...
  return (
    <div className={`flex flex-col ${hidden && 'hidden'}`}>
      <div
        id="boom"
        className={`absolute right-20 top-8 z-50 translate-x-[2px] text-8xl  text-primary blur-[4px] transition-all ${channelChange && 'scale-110 saturate-150'}`}
      ...
      <div
        id="boom"
        className={`text-["littlebit-dotty-variable", sans-serif] absolute right-20 top-8 z-50 text-8xl text-primary blur-[1px] transition-all ${channelChange && 'scale-110 saturate-150'}
        `}
        style={{ fontVariationSettings: `"BLOC" 500, "OPEN" 100` }}
      >
        {inputChannel ? returnCH : String(currentChannel).padStart(2, '0')}
